...
<style>
    *, *::before, *::after { box-sizing: border-box; }
    ...
    .btn-update { display: flex; align-items: center; justify-content: center; gap: 6px; padding: 11px 28px; background: #b8860b; color: #fff; font-size: 14px; font-weight: 700; border-radius: 10px; border: none; cursor: pointer; transition: background .2s; }
    .btn-update:hover { background: #9a7209; }
...
<div class="main-wrapper">
    ...
    <div class="container-fluid px-4 px-md-5 py-4">
        ...
        <form method="POST" action="{{ route('master.layanan.update', $layanan->id) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-card">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="kategori_id" id="kategoriSelect" class="form-select">
                        ...
                        @foreach($kategoris as $kat)
                            <option value="{{ $kat->id }}"
                                {{ old('kategori_id', $layanan->kategori_id) == $kat->id ? 'selected' : '' }}>
                                {{ $kat->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="judul_surat" class="form-control" value="{{ old('judul_surat', $layanan->judul_surat) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Judul Surat (Inggris)</label>
                    <input type="text" name="judul_surat_en" class="form-control" value="{{ old('judul_surat_en', $layanan->judul_surat_en) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Kalimat Pembuka (Indonesia) <span class="text-muted">— kosongkan jika tidak ada</span></label>
                    <textarea name="kalimat_pembuka" class="form-control" rows="2">{{ old('kalimat_pembuka', $layanan->kalimat_pembuka) }}</textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Kalimat Pembuka (Inggris)</label>
                    <textarea name="kalimat_pembuka_en" class="form-control" rows="2">{{ old('kalimat_pembuka_en', $layanan->kalimat_pembuka_en) }}</textarea>
                </div>
                <div class="col-12">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $layanan->is_active) ? 'checked' : '' }}>
                ...
                <div class="col-12">
                    ...
                    <div class="d-flex flex-wrap gap-4">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="show_dokter_section" id="show_dokter_section" value="1" {{ old('show_dokter_section', $layanan->show_dokter_section) ? 'checked' : '' }}>
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="show_nama_dokter" id="show_nama_dokter" value="1" {{ old('show_nama_dokter', $layanan->show_nama_dokter) ? 'checked' : '' }}>
                            <label class="form-check-label" for="show_nama_dokter" style="font-size:13px; font-weight:500;">Tampilkan field Nama Dokter</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="show_nama_suami" id="show_nama_suami" value="1" {{ old('show_nama_suami', $layanan->show_nama_suami) ? 'checked' : '' }}>
                            <label class="form-check-label" for="show_nama_suami" style="font-size:13px; font-weight:500;">Tampilkan field Nama Suami</label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="show_bangsa" id="show_bangsa" value="1" {{ old('show_bangsa', $layanan->show_bangsa) ? 'checked' : '' }}>
        ...
        {{-- BIODATA FIELDS --}}
        @if(!$layanan->template_path)
        <div class="alert" style="background:#fff8e1; border:1.5px solid #ffe082; border-radius:12px; padding:14px 18px; font-size:13.5px; color:#7a5c00; margin-bottom:0;">
            <i class="bi bi-info-circle-fill me-2"></i>
            Layanan ini berasal dari pengajuan <strong>Lain-lain</strong> dan hanya mendukung upload file.
            Konfigurasi template surat tidak tersedia.
        </div>
        @else
        <div class="form-card">
            ...
            <div id="biodata-container">
                @forelse($layanan->biodataFields as $field)
                <div class="biodata-row">
                    ...
                    <div class="row g-2">
                        <div class="col-md-3">
                            ...
                            <input type="text" name="biodata_label[]" class="form-control" value="{{ $field->label }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Label (EN)</label>
                            <input type="text" name="biodata_label_en[]" class="form-control" value="{{ $field->label_en }}">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Field Key</label>
                            <input type="text" name="biodata_field_key[]" class="form-control" value="{{ $field->field_key }}">
                            <div class="field-key-hint">Nama kolom di database</div>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Suffix (ID)</label>
                            <input type="text" name="biodata_suffix[]" class="form-control" value="{{ $field->suffix }}">
                        </div>
                        <div class="col-md-2">
                            <label class="form-label">Suffix (EN)</label>
                            <input type="text" name="biodata_suffix_en[]" class="form-control" value="{{ $field->suffix_en }}">
                        </div>
                        <div class="col-md-2 d-flex align-items-end pb-1">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="biodata_is_admin[]" value="1" {{ $field->is_admin_field ? 'checked' : '' }}>
        ...
        {{-- ISI SURAT, PENUTUP, TTD — hanya untuk layanan dengan template --}}
        {{-- ISI SURAT --}}
        <div class="form-card">
            ...
            <div class="mb-3">
                ...
                <textarea name="isi_id" class="form-control" rows="5">{{ old('isi_id', $layanan->isiTemplate?->isi_id) }}</textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Isi Surat (Inggris)</label>
                <textarea name="isi_en" class="form-control" rows="5">{{ old('isi_en', $layanan->isiTemplate?->isi_en) }}</textarea>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <textarea name="kalimat_penutup" class="form-control" rows="2">{{ old('kalimat_penutup', $layanan->kalimat_penutup) }}</textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label">Penutup (Inggris)</label>
                    <textarea name="kalimat_penutup_en" class="form-control" rows="2">{{ old('kalimat_penutup_en', $layanan->kalimat_penutup_en) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="ttd_kiri_label" class="form-control" value="{{ old('ttd_kiri_label', $layanan->ttd_kiri_label) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Label TTD Kiri (EN)</label>
                    <input type="text" name="ttd_kiri_label_en" class="form-control" value="{{ old('ttd_kiri_label_en', $layanan->ttd_kiri_label_en) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Label TTD Kanan (ID) <span class="text-muted">— kosongkan jika tidak ada</span></label>
                    <input type="text" name="ttd_kanan_label" class="form-control" value="{{ old('ttd_kanan_label', $layanan->ttd_kanan_label) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">Label TTD Kanan (EN)</label>
                    <input type="text" name="ttd_kanan_label_en" class="form-control" value="{{ old('ttd_kanan_label_en', $layanan->ttd_kanan_label_en) }}">
                </div>
                <div class="col-md-6">
                    <label class="form-label">TTD Kanan diisi oleh</label>
                    <select name="ttd_kanan_sumber" class="form-select">
                        <option value="persetujuan" {{ old('ttd_kanan_sumber', $layanan->ttd_kanan_sumber) === 'persetujuan' ? 'selected' : '' }}>Nama Persetujuan (diisi admin)</option>
                        <option value="dokter" {{ old('ttd_kanan_sumber', $layanan->ttd_kanan_sumber) === 'dokter' ? 'selected' : '' }}>Nama Dokter (dari pengajuan)</option>
                    </select>
                </div>
                <div class="col-12">
                    ...
                    <select name="ttd_tanggal_posisi" class="form-select">
                        ...
                        <option value="kiri" {{ old('ttd_tanggal_posisi', $layanan->ttd_tanggal_posisi) === 'kiri' ? 'selected' : '' }}>Di atas TTD Kiri</option>
                        <option value="kanan" {{ old('ttd_tanggal_posisi', $layanan->ttd_tanggal_posisi) === 'kanan' ? 'selected' : '' }}>Di atas TTD Kanan</option>
                    </select>
                </div>
            </div>
        </div>

        @endif {{-- end: template_path check --}}

        <div class="d-flex gap-3 mb-5" style="margin-top: 24px;">
            <button type="submit" class="btn-update"><i class="bi bi-save"></i> Update Layanan</button>
...
function addBiodataRow() {
    ...
    row.innerHTML = `
        ...
        <div class="row g-2">
            ...
            <div class="col-md-2"><label class="form-label">Suffix (ID)</label><input type="text" name="biodata_suffix[]" class="form-control" placeholder="Tahun"></div>
            <div class="col-md-2"><label class="form-label">Suffix (EN)</label><input type="text" name="biodata_suffix_en[]" class="form-control" placeholder="years old"></div>
            <div class="col-md-2 d-flex align-items-end pb-1"><div class="form-check"><input class="form-check-input" type="checkbox" name="biodata_is_admin[]" value="1"><label class="form-check-label" style="font-size:12px;">Diisi Admin</label></div></div>
        </div>`;
